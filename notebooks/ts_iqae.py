...
def read_stack(sc, file_df):
    all_images = sc.parallelize(
        [(ij['Time(hrs)'], ij['path']) for ij in file_df.sort_values(by='Time(hrs)').T.to_dict().values()]).filter(
        lambda (_, path): os.path.exists(path)).sortByKey()
